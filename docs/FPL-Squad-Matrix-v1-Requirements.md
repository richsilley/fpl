...
#### Badges name the benefit

Each move carries the checks it passed, worded as what the reader gains rather than which rule fired: **More points**, **Better value**, **Long-term pick**. "Raises the floor" was poker jargon for the worst realistic outcome and read as nothing at all to anyone outside that world.

The rule itself, with its threshold, is on hover. Badge and tooltip together give plain language at a glance and the exact number on demand, without either having to be sacrificed for the other. The rejection sentence draws its names from the same source, so one check cannot appear under two names.

#### Best available, by position

Replaces a "considered and cut" list of rejections, which was interesting to whoever wrote the rules and confusing to everyone else. A reader wants to know who to buy, not who an algorithm declined to suggest.

Three goalkeepers and five of each outfield position, ranked on projected points over the horizon, **excluding players already owned** and **ignoring price entirely**. The packages above already answer what is affordable today; this answers what is worth reaching for, which is the question that decides whether to sell two players to fund one. Filtering by budget would hide exactly those targets.

Availability, minutes and the obscurity rule still apply — an injured or fringe player is not a pick at any price.

Each row shows price, projected points, fixture score and ownership within the selected scope as **numbers rather than bars**: this is a shortlist to scan, and the bars belong on the Ownership view where comparing fifteen of them is the job. A one-line benefit names the single strongest thing about the player, so one suggested for fixtures reads differently from one suggested for form.

#### The player panel

Clicking any suggested player — in the picks list or as the incoming side of a package — opens a floating panel showing, in order: ownership with bars as the Ownership view draws them, the fixture matrix over the selected horizon as the Fixtures view draws it, and the form figures as the Form view shows them.

The Edge suggests players the reader has never looked at. Sending them to three other views to check a suggestion is the worst possible ask at the moment they are deciding, so the three answers come to them. It reuses the other views' rendering rather than restating it, so a fixture that reads amber there cannot read green here.

URL state (`?player=<id>`) like every other overlay, so it dismisses on Escape and on the backdrop.
...
**Scope, then horizon, then transfers, all visible.** Each narrows the question the next one answers. The horizon sat behind an Advanced disclosure for a while, which hid the control that most changes what the suggestions are. **Difficulty is fixed to Form** — the gate reads fixtures through the projection, and offering three ratings for a number the reader never sees was a control over nothing.
